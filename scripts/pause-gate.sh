...
if [[ -f "$AITEAMOS_PAUSE_FLAG" ]]; then
  # AITEAMOS_PAUSE_REASON is an optional per-run override for the operator-
  # facing deny message -- the dynamic reason (a task key, an operator's
  # name, the operator's own free-text message) that ADR 0001 section 7
  # anticipates a caller will want to supply, once one exists. Unset or
  # empty falls back to the original static message, so a caller that never
  # sets it (every caller today) sees exactly the old behaviour.
  deny "${AITEAMOS_PAUSE_REASON:-Paused by AI Team OS. Stop and wait.}"
fi
