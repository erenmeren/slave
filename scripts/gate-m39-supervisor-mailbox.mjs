...
try {
  ...
  const qa = await prisma.slave.create({ data: { teamId: team.id, role: 'QA Engineer', runtimeRoles: [ASKED_ROLE], personId: (await prisma.person.upsert({ where: { name: 'Quinn' }, create: { name: 'Quinn' }, update: { templateId: null, profile: null, model: null, provider: null, capabilities: [], lifecycle: 'project', releasedAt: null, releaseReason: null, selectionRationale: null } })).id } })
  console.log(
  ...
  const ops = await prisma.slave.create({ data: { teamId: team.id, role: 'Platform Engineer', runtimeRoles: ['backend'], personId: (await prisma.person.upsert({ where: { name: 'Ops' }, create: { name: 'Ops' }, update: { templateId: null, profile: null, model: null, provider: null, capabilities: [], lifecycle: 'project', releasedAt: null, releaseReason: null, selectionRationale: null } })).id } })
