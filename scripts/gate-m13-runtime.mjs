...
try {
  ...
  // ============================================================================================
  const attemptSlave = await prisma.slave.create({ data: { teamId: team.id, role: 'stage5', runtimeRoles: ['stage5'], model: CLAUDE_MODEL, provider: 'claude_code', personId: (await prisma.person.upsert({ where: { name: ATTEMPT_WORKER }, create: { name: ATTEMPT_WORKER }, update: { templateId: null, profile: null, model: null, provider: null, capabilities: [], lifecycle: 'project', releasedAt: null, releaseReason: null, selectionRationale: null } })).id } })
