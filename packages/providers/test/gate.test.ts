 ...
 * `permission matrix denies '<capability>' (<tool>) for this slave` (scripts/lib/permissions.sh),
...
describe('parsePermissionDenyReason', () => {
  it('parses the fixed grammar into tool and capability', () => {
    expect(parsePermissionDenyReason(`${PERMISSION_DENY_REASON_PREFIX} 'run tests' (Bash) for this slave`)).toEqual({
      tool: 'Bash',
  ...
  it('returns null, never throws, on a reason that starts with the prefix but is malformed past it', () => {
    ...
    expect(parsePermissionDenyReason(`${PERMISSION_DENY_REASON_PREFIX} run tests (Bash) for this slave`)).toBeNull()
