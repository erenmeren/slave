...
describe('preflightGate', () => {
  ...
  beforeEach(() => {
    ...
    dir = mkdtempSync(path.join(tmpdir(), 'aiteamos-preflight-'))
    flagPath = path.join(dir, 'pause.flag')
  ...
  it('preflight accepts a hook that discriminates', async (): Promise<void> => {
    await expect(preflightGate({ hookPath, flagPath })).resolves.toBeUndefined()
  })

  it('leaves no flag file behind either way', async (): Promise<void> => {
    await preflightGate({ hookPath, flagPath })
