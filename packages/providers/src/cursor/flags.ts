 ...
 * gate". The probe itself lives in `runtime/gate-preflight.ts` and is shared with Claude: it spawns
 * `gatePath` directly (never through `cursor-agent`), twice, against an isolated temporary flag
 * file it mints itself, and asserts BOTH directions:
 *
 ...
 *
 * **The allow half differs from Claude's, and that difference is the only reason the shared probe
 * takes an `expectAllow` at all** (Task 11 §8(d)). Claude's gate allows by staying SILENT. Cursor
 * classifies exit 0 with empty stdout as a hook FAILURE (`empty_stdout`), which `failClosed: true`
 * converts into a block -- so a Cursor gate must speak its allow out loud, and a pre-flight that
 * asserted Claude's silent allow would reject a correct one.
 *
