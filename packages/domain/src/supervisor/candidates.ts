...
function readTaskFailure(task: SupervisorTask): FailureDiagnosis {
  return readFailure({
    ...
    // H9b R1: off the row, so a platform failure reads as infrastructure even when its reason
    // line names nothing the regular expression knows.
    failureClass: task.latestFailure?.failureClass ?? null,
  })
}
