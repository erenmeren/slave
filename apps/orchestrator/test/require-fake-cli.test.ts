 ...
 *
 * M56a R10 widens it to EVERY registered provider, which is why every "allows" case below now names
 * both binaries: with the flag set, a valid environment is one in which no vendor's real CLI can be
 * reached, not one in which Claude's cannot.
 */
...
describe('fakeCliRefusal', () => {
  ...
  it('allows the shapes every fake in this repo actually takes, once BOTH are named', () => {
    ...
    // ...and the executable scripts CI points at.
    expect(
      fakeCliRefusal({
        SLAVEOFAI_REQUIRE_FAKE_CLI: '1',
