import { setSlaveModel, type ProviderKind } from '@slave-of-ai/control'
...
export async function POST(
  ...
  // one -- both mean "the operator did not name a provider" -- so `setSlaveModel` still gets
  ...
  return orgControlResponse(() =>
    setSlaveModel(slaveId, model, provider as ProviderKind | null, gate.principal ?? undefined),
  )
}
