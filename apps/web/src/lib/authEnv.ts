/**
 * The one place `AITEAMOS_SESSION_SECRET` is read (M23 spec §7 F1). Empty or absent → loopback
 * mode, M15 byte for byte; a non-empty value → accounts mode, where the app answers to any Host
 * and every request carries a user-bound session cookie. `AITEAMOS_PASSWORD` is retired with M20's
...
export function sessionSecret(): string | null {
  const raw = process.env['AITEAMOS_SESSION_SECRET']
