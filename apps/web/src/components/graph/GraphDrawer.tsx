...
export function GraphDrawer({
  ...
  return (
    // M61 Task 10: the 352px aside itself stays fixed (`docs/ia.md`'s own layout, unchanged) --
    // only its raw `overflow-y-auto` becomes `ui/ScrollArea` (`min-h-0` on the aside is what lets
    // that inner region actually shrink to the row's bounded height rather than grow to fit).
    <aside data-testid="graph-drawer" aria-label="Slave detail" className="flex min-h-0 w-[352px] shrink-0 flex-col border-l border-line bg-bg-1">
