...
export function CompareClient({ comparison }: { readonly comparison: SimulationComparison }): React.JSX.Element {
  ...
  return (
    // M61 Task 10: `min-h-0`, not `min-h-screen` -- the strip below stays fixed in this same
    // padded, gapped column (unchanged), and everything after it is the page's own scrolling
    // body, in `ui/ScrollArea` now.
    <div className="flex min-h-0 flex-1 flex-col gap-4 p-6">
      ...
      {/* The inner `gap-4` reproduces exactly the gap this content had as direct siblings of the
        * strip above, so nothing moves a pixel. */}
      <ScrollArea>
