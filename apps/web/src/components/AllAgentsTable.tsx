...
import type { AllAgentRow, AllAgentsPage } from '../server/org'
...
/** M24 §5.3's one Agents table: agent · role · department · project · status · current task ·
...
const HEADER = ['Agent', 'Role', 'Department', 'Project', 'Status', 'Current task', 'Provider', 'Cost', ''] as const
 ...
 *
 * `initial` is the full `AllAgentsPage` now, not a bare row array (M25 Task 6, spec §4.1): the
...
export function AllAgentsTable({
  ...
  readonly initial: AllAgentsPage
  readonly onOpen: (row: { readonly agentId: string; readonly workspaceId: string }) => void
}): React.JSX.Element {
  const [rows, setRows] = useState<readonly AllAgentRow[]>(initial.rows)
    ...
    async function poll(): Promise<void> {
      ...
      try {
        ...
        setRows((prev) => {
          ...
          // `companyAgentId`/`companyId`/`companyTeamId`/`model` are `null` -- none is knowable
...
/**
 * The Agents table's department cell (M25 Task 6, spec §4.1): a `<select>`, not a bare name --
 * a project row lists its own workspace's departments and PUTs `/api/agents/:id/team`
 * (`{ teamId }`); a catalog row lists its own company's templates (its `CompanyTeam`s) and PUTs
 * `/api/org/agents/:id/team` (`{ companyTeamId }`). `isProject` is `AllAgentsTable`'s own project-
 * row test (`agentId !== null && workspaceId !== null`), repeated here rather than threaded down
 * as a prop -- the two facts it reads (`row.agentId`, `row.workspaceId`) are already on `row`.
 *
 ...
 * firing its own `onChange`. A refusal renders `agent-department-error` under the select instead
