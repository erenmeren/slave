...
export function AgentRowActions({
  ...
  const commit = async (field: 'name' | 'role'): Promise<void> => {
    ...
    setPending(true)
    setErrorText(null)
    const path = `/api/agents/${agentId}/${field}`
  ...
  const remove = async (): Promise<void> => {
    setPending(true)
    setErrorText(null)
    const error = await sendControl(`/api/agents/${agentId}`, { method: 'DELETE' })
    setPending(false)
  ...
  return (
    <div data-testid="agent-row-actions" className="flex flex-wrap items-center gap-1">
      {editing === 'name' ? (
        <TextField
          inputProps={
            {
      ...
      {confirmingDelete ? (
        <>
          <PrimaryButton tone="blocked" data-testid="agent-delete-confirm" disabled={pending} onClick={() => void remove()}>
        ...
        <PrimaryButton tone="blocked" data-testid="agent-delete" disabled={pending} onClick={() => setConfirmingDelete(true)}>
          delete
        </PrimaryButton>
      )}
      {errorText !== null && (
        <span role="alert" data-testid="agent-actions-error" className="text-xs text-tone-blocked">
          {errorText}
        </span>
      )}
    </div>
