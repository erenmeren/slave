...
/**
 * The login page itself (M20 spec §3.3), not the form inside it. Two things had no consumer
...
describe('LoginPage', () => {
  ...
  it('says there is nothing to log in to on a loopback-only instance, and offers the way back', async () => {
    ...
    expect(notice.textContent).toContain('password login is not configured on this instance')
    expect(screen.queryByTestId('login-form')).toBeNull()
    expect(within(notice).getByRole('link').getAttribute('href')).toBe('/')
  })

  it('renders the form, not the notice, once a password is configured', async () => {
