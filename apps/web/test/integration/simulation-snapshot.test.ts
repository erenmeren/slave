...
describe('buildSimulationSnapshot', () => {
  it('reads the company panel, the metrics, the last journal rows and an empty model-usage panel', async () => {
    ...
    expect(snapshot?.sector).toBe('trade')
    expect(snapshot?.currency).toBe('USD')
    // M31b Task 4: the "company" panel is the sector's own `headline`, not a trade-shaped object.
    expect(snapshot?.headline.find((h) => h.label === 'day')).toMatchObject({ value: 30 })
    expect(snapshot?.headline.find((h) => h.label === 'inventory')).toMatchObject({ value: 50 })
    expect(snapshot?.headline.find((h) => h.label === 'open orders')).toMatchObject({ value: 0 })
    expect(snapshot?.metrics['deliveredQty']).toBe(150)
    expect(snapshot?.metricLabels['deliveredQty']).toEqual({ label: 'delivered', kind: 'count' })
  ...
  it('is null for an unknown id; the cards list and the company list read the catalog', async () => {
    ...
    expect(await listSimulationCompanies('trade')).toEqual([{ id: companyId, name: 'Demo Trading Co.', slaves: 4 }])
  })
})
