...
9. §5/§7: `gate:m15-boundary` run standalone now requires `AITEAMOS_PASSWORD` to be unset in the environment (in password mode its stage 1 receives 302/401 rather than 403); `gate:m20-auth` run A deletes the variable from the child environment for exactly this reason. — closed by M21 A1 (the M15 gate blanks the variable for its own child).
