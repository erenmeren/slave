...
- **Compare (Task 3, backlog) — DONE in M32 item 5:** `COMPARED_KEYS` in control's `read.ts` was a hand-made union of the sectors' world keys. `SectorPlugin` carries `comparedKeys: readonly string[]` now and control reads `plugin.comparedKeys`; the union is deleted. Trade lists `roster, roles, initial, scenario, currency, horizonDays, limits` (exactly what the union listed for it); software lists `roles, engineers, scenario, currency, horizonDays, limits`. The conformance test asserts every entry is a real field of that sector's demo definition — the failure the union hid, since a key a sector does not carry reads `undefined === undefined` on both sides and compares nothing.

  Software's `reviewCapacityPerDay` / `reviewEverything` / `matchWaitDays` are deliberately NOT listed, and this is a documented deviation from the M32 brief, which named them. They are `POLICY_SETTINGS[policy]` verbatim and `cloneDefinition` overwrites them from the new policy — they ARE the policy, under three other names. Listing them would make every A-vs-B comparison report "the runs did not share the same world", which is the one thing M30 §4 excludes `policy` and `seed` to prevent, and would contradict both `software.test.ts`'s clone-pair assertion and `gate:m31b-software-sector`'s own stage-4 note.
- **`SimulationMetrics = Record<string, number>` was a runtime approximation (backlog) — DONE in M32 item 6:** control published a sector's metrics under that type while trade's `tradeMetrics` returns `sources` (an object) and `minCashDay` beside the numbers, and two `as SimulationMetrics` casts told the compiler otherwise. It is `Readonly<Record<string, unknown>>` now, both casts are gone, and the narrowing lives where a number is actually needed: a new pure `metricDeltas(metricLabels, a, b)` in control's `read.ts` computes `b − a` over the LABELLED keys only (so `sources` is never subtracted from `sources`), counting a missing key as 0 and yielding `null` — not 0 — for a labelled key that is not a finite number. `SimulationComparison.deltas` is therefore `Record<string, number | null>` and the compare page draws `—` for a null figure or delta, by the same narrowing rule, so a row's A, B and Δ cannot disagree about what is a number. `metricDeltas` is unit-tested against a fabricated label set, since both real sectors label numbers only — which is exactly why the cast went unnoticed.
