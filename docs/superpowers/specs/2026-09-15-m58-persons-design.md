...
Execution-time (Task 2 review, 2026-09-15):

- **E8 (amends R8 and the plan's migration SQL)** — the backfill's name de-duplication assigns suffixes against the set of ALL names already taken (seat-born, roster-born and those assigned so far), looping until no duplicate remains; the independent per-partition `" 2"` rule could collide with a pre-existing `"<name> 2"` and abort the migration on legal data. The migration test's fixture carries that shape.
- **E9 (amends E5)** — the `materialise_company_worker` action arm is WIDENED, not renamed: it accepts `personId` or the legacy `companySlaveId` (one required), so a decision row of any status and every entry of a stored `candidates` list still parse after the upgrade; pending rows' `action` is still rewritten to `personId` by the migration; a legacy `companySlaveId` reaching the apply path is refused (it cannot resolve any more).
