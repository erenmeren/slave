...
- Before spawning, `preflightGate({ hookPath })` (the existing pattern: the hook must answer `deny`); the settings file is written with `writeSettingsFile` into the temp dir — the deny-all hook is the existing adapter hook run in a mode that denies everything: `SLAVEOFAI_DENY_ALL=1` in the child env makes the current hook script deny without reading a matrix file (a one-branch change to the hook, tested).
