...
- **R7 (Task 4):** `tickSimulations` makes at most ONE model call per pass (later due llm runs wait for the next pass; rules runs in the same pass are unaffected) so a slow call bounds the daemon loop by one timeout, not fifty. Off-loop llm stepping with an in-flight set keyed by run id is M31 backlog.
