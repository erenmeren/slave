...
- **E12 (amends R13 and §7 stage 5)** — merged M58's `createPerson(input, principal?)` returns `{ personId, name }`, not `{ id }`, and `assignPerson(personId, teamId, options?, principal?)` accepts the principal after its optional role/runtime-role/engagement options; intake staffing uses `personId` and passes its principal through both verbs, while the gate reads a seat's name through `Slave.person.name` because `Slave.name` moved to `Person.name`.
