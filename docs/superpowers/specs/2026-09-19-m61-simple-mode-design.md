...
- **E2 (amends R8)** — `OrganizationRow` already carries `doing: string | null`, `why`, `lifecycle` and `released`; `TeamLiveRow.doing` prefers `SlaveCardData.taskTitle` (a live run's task) and falls back to `OrganizationRow.doing`, then to the waiting/idle sentences R8 lists. `SlaveCardData` also already carries `progressPct`, so `TeamLiveRow.progress` is `PROGRESS_FOR_TASK_STATUS[taskStatus]` when the task is not `running` and `35 + progressPct * 0.25` (35→60) while it is.
...
- **E7 (amends R21)** — the fixture in `gate-m57-ui-redesign.mjs` (two workspaces, five tasks, a pending decision, two goal events, a simulated company) is exactly the fixture stages 2–10 of the new gate need; `gate-m61-simple-mode.mjs` copies its Stage 0 verbatim with `M61` names and adds one live `SlaveRun` in `working` on the seeded slave so stage 5 has a `data-progress > 0` card.
