...
### Task 3 (2026-09-09)
- **E5 — the runtime-roles refusal kind.** §6 said `setRuntimeRoles` refuses with `invalid_role`. That kind already exists, payload-less, for `setSlaveRole` (the title). Corrected: `setRuntimeRoles` refuses with `invalid_runtime_roles { reason }` (blank entry, duplicate, more than `MAX_RUNTIME_ROLES = 20`), so a title refusal and a dispatch-set refusal stay distinct and the CLI/web can show the reason (controller ruling).
- **§6 clarified — actor.** Both verbs take a plain `actor: string` and record it in the event payload; they do not take a `Principal`, so `ExecutionEvent.userId` stays null for `slave.profile_changed` and `slave.runtime_roles_changed`. The web passes the principal's user id as the actor. Threading `Principal` through is a follow-up.
### Final review (2026-09-09)
- **E6 — `PLANNING_GRAPH_INSTRUCTIONS` is verbatim except one word.** Task 1 moved the planning trailer out of `buildPlanningPrompt` "verbatim". It is not, by one word: `below` → `above`. `buildPlanningPrompt` printed the instructions and then the goal, so "Decompose the GOAL below" pointed forward; §3's `SECTION_ORDER.planning = ['profile','planning_goal']` puts the goal ABOVE the trailer, which the renderer appends last, so the unchanged sentence would have pointed at nothing and contradicted the prompt it is part of. Both fake-CLI selectors survive the change: the planning arm still keys on `"task graph"`, and the text still carries no `"verdict"`. `packages/domain/test/run-context/render.test.ts` asserts the deviation explicitly rather than folding it into the expected text.
