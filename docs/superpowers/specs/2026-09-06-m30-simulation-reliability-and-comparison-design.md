...
The rulings made while executing Tasks 1–5, and the notable execution findings alongside them, in order:

- **R1** — the barrel (`packages/control/src/simulation.ts`) re-exported `./simulation/shared.js`, `./simulation/read.js` and `./simulation/write.js` but not `./simulation/auto-run.js`, because that file did not exist yet — Task 3 was the one that created it. Ruling: correct as delivered; the omission was not a defect for Task 1 to fix, it was a dependency Task 3's own dispatch carried forward as an instruction to add the export line once the file it names existed to export.
- **R2** — `createSimulation` and `cloneSimulation` each carried their own copy of the same validate-then-`create`-then-catch-the-unique-violation block, differing only in which fields they wrote. Ruling: extracted into a private `insertRun(data, journalPayload, name)` helper in `write.ts` that wraps the create and turns a `(companyId, name)` collision into the typed `duplicate_name` refusal — both verbs' one non-trivial piece now lives in one place.
- **R3** — Task 3's own planned test named the wrong refusal reason: after a run is paused, `stopAutoRun`'s intent-clearing means a later `startAutoRun` on that same paused run is refused outright, so `autoStepDue` never gets far enough to answer `not_running` — it answers `no_intent` first, because there is no intent left to be not-running against. Ruling: the corrected assertion was accepted as right; `not_running` still needed its own coverage, reached by flipping a run's status to `paused` directly through Prisma while its auto-run intent stays armed — the exact state `autoStepDue`'s guard is written to catch.
- **R4** — `haltUnparsed` (the halt path for a run whose stored state fails to parse) left `state.journalSeq` behind the journal's own `max(seq)`, since it deliberately never rewrites the corrupt `state` column; and `injectExternalEvent` carried no status guard at all, so injecting against a run a throwing step had just halted collided on a stale sequence number (`P2002`) instead of answering a refusal. Ruling: `injectExternalEvent` now guards to `status ∈ {ready, running}` (`simulation_not_runnable` otherwise) and derives its seq from a fresh `max(seq) + 1` read under the row lock rather than trusting `state.journalSeq` — belt and braces, since the status guard alone already makes a halted row unreachable here. `haltUnparsed` itself stays state-free: the corrupt JSON is the evidence of what went wrong, not something a halt should try to repair.
- **R5** — two independent gaps in the streaming and control surface: the heartbeat's SSE enqueue carried none of the try/catch-and-close a normal message enqueue already had (a client disconnected mid-heartbeat could throw past it); and `Pause` was disabled by the same `runnable && autoRun === null` guard as `Step`/`Run to day`, when §4 makes pausing exactly what should be able to stop an auto-run in flight. Ruling: fixed both — `Pause` stays enabled whenever `status ∈ {ready, running}` regardless of an auto-run intent, while `Step`/`Run to day` additionally require no intent (a manual step during auto-run is a UI refusal, not the control verb's); the heartbeat interval also picked up `.unref()` so it cannot hold the process open on its own.
- **R6** — three findings against the compare page: the compare strip's copy omitted the sector the plan's own wording named; `buildSimulationSnapshot`'s read of compare candidates ran outside the `RepeatableRead` transaction the rest of the snapshot uses, open to the same stale-journal race the M29 spec's own R6 already closed for `replaySimulation`/`simulationStatus`; and the journal-read-plus-`tradeMetrics` computation was triplicated across `simulationStatus`, `compareSimulations` and `replaySimulation`'s injected-event count. Ruling: fixed all three — the strip shows `trade`; candidate listing gained a client parameter so it can run inside the same transaction; a shared `loadSideMetrics(tx, id)` now backs all three read paths.

M31 backlog (restated here as what comes next): the LLM decision provider needs a text-only,
zero-tool CLI spawn whose isolation from the software-workspace path is proven, not assumed — a
decision call must never be able to reach a tool, a hook, or a real repository; a software-company
sector alongside trade's; and the minors this milestone's review deferred rather than folded in —
the policy `<option>` labels duplicated between the new-simulation and clone drawers (a shared
`PolicySelect`); `autoStepDue` checking `not_due` before `until_day` (costs one extra due-but-
already-past-horizon cycle before the intent clears); `stopAutoRun` and `autoStepDue` sharing two
