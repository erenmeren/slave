...
Controller rulings made while executing Tasks 1–5 and the whole-branch final review (R12–R16), in order:
...
- **R6** — no confetti trigger in M28: confetti is **deferred**, not impossible. The signal exists — `apps/orchestrator/src/pump.ts` appends `run.succeeded` with the run's `slaveId`, and `useOverview` keeps `{ seq, ts, type, summary }` per `slaveId` in `liveEvents` — so a trigger would read `liveEvents[slaveId]` for a `run.succeeded` type in the client, not in `liveOffice.ts`: the snapshot the adapter is handed carries statuses and counts, not who just succeeded, and `progressPct` is a tool-call ratio rather than a finish line. The engine's confetti/coin machinery stays vendored and unused, and the design's "working → idle at 100" trigger and its test are dropped, as a scope call. See also §8's reduced-motion note below — moot without a trigger, but recorded as deferred rather than silently dropped.
...
- **R12** — the HUD's `working` count reads the stream, not the floor: `world.slaves.filter((s) => world.liveOf(s.id)?.status === 'working').length`. `world.status(s)` answers from the sprite's state, so a live-working slave still walking back from the arcade (and a `starting` one walking to the board) went uncounted, and `office-hud-counts` disagreed with the Overview tab for the length of every walk.
- **R13** — a lounge break ends early when the status changes: `simulate`'s `coffee`/`arcade` case returns to the desk on `slave.timer <= 0 || this.statusOf(slave) !== 'idle'`. `reconcile` only runs for seated states, so without the second test a slave the stream reports `blocked` stood at the arcade with no bubble for the rest of the 3.5 s / 5 s break plus the walk home.
- **R14** — R6's original wording ("the stream carries no per-slave success signal") was false and is corrected above: `pump.ts` appends `run.succeeded` with a `slaveId` and `useOverview` exposes it per slave in `liveEvents`. Confetti stays dropped for M28 as a scope call; the adapter's class docstring says "deferred, and here is where the trigger would read from" rather than "impossible".
- **R15** — `scripts/gate-m11-shell.mjs` stage 7 opens project **B**'s office, not project A's. Stage 6 leaves A with one department and zero slaves, so the browser gate drew an empty floor, rendered no focus card and never clicked `Next` — it exercised none of what M28 added. B is still active and still holds its materialized worker; the stage now also creates one extra slave row directly with prisma (`M11 Gate Second Slave`, role `qa`, in B's department — a fixture in the same spirit as stage 6a's `slaveRun` fixture) so the floor carries two slaves and the `Next` branch actually runs. The three-way roster-size guard stays for robustness, with a hard failure below two slaves so the cycle assertion can never be skipped silently. The script's existing cleanup deletes workspace B and cascades the fixture away.
- **R16** — the minor pass taken with R12–R15, all in the same wave: `FocusCard` offers `Resume` only while the status is `paused` (`pausing` keeps the label `Pause` and disables the button, per §5); `OfficeClient` guards `document.fonts` before `load` (starting the loop straight away when there is no font set); the rebuild effect seeds `setWallClock(wallHour())` before `apply` so the HUD does not read the engine's initial 09:00 through the font-load window, and `LiveOffice.setWallClock` now lands the hour immediately (`this.hour = this.hourLock ?? hour`) rather than only at the next `tick`, which is what makes that seeding visible; the pre-effect HUD falls back to the server roster for both counts (`initial.departments.length`, and their slaves summed) instead of `0 slaves`; and the loop's `tick`/`render` catch reports only the first throw, since a broken frame throws sixty times a second.
...
- Confetti: no trigger at all in M28 (R6, as corrected by R14 above, supersedes the plan's "working → idle at 100" note — deferred, with the signal named).
