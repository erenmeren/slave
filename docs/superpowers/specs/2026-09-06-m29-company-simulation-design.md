...
The rulings made while executing Tasks 1–11, and the notable execution findings alongside them, in order:
...
- **R5** — an inject → step-past-it → replay test was added to Task 6's scope, pinning the contract for where an injected event lands relative to the scenario's own scheduled events: `replaySimulation` seeds injected events into the initial queue AFTER the scenario's, so the external-class tie order (scenario first, then injections in the order they were made) matches what a live run's queue actually built up to. The ruling was to fix the code only if that test failed against the engine's existing behaviour; it passed unchanged, so this confirmed an ordering the engine already produced rather than changing one.
- **R6** — `replaySimulation` and `simulationStatus` read a run's row and its journal inside one `RepeatableRead` transaction, not two unlocked queries: under the default Read Committed isolation a concurrent `stepSimulation` commit landing between the two reads could make the journal newer than the state it is compared against, reporting a spurious replay mismatch that was never real.
- **R7** — the state-vs-journal comparison inside `replaySimulation` sorts object keys at every level before stringifying (`stableStringify`), rather than comparing raw `JSON.stringify` output: Postgres reorders a stored `jsonb` object's keys by length then alphabetically, so a live engine state and the same state round-tripped through storage could disagree by key order alone with zero difference in content. Found and fixed during Task 6's fix round rather than called out as a review finding, but it is as much a ruling as the four above it — the comparison would otherwise manufacture false mismatches on real data.
- **R8** — the halt route treats an empty request body as `{}` (no reason given) but answers 400 on a non-empty, unparsable one: pause and resume take no body at all, and a bodiless halt from a script is a legitimate call the same way; a syntactically broken body is never legitimate and should not silently become "no reason."
- **R9** — `create-simulation --seed <n>` is validated at both the CLI and the `createSimulation` verb: an unparsed seed would otherwise persist as `null` and make every later verb on that run fail with `simulation_corrupt`. The verb is the boundary every caller (CLI today, routes and any future caller) shares, so the refusal lives there; the CLI additionally throws its own message before ever calling it, since a friendlier error at the command line beats a generic one from the verb.
