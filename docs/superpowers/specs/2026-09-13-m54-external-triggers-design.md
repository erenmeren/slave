...
Twenty-five, each as `**En (amends Rx)** — <one-line claim>.` with its reasoning and file
...
each names the round that produced it; **E24–E25** were written by the ONE fix wave that answered
the final whole-branch review's Critical and its first Important.
...
- **E24 (amends R8, §3 stage 9 and §2's `ExternalOrigin`, from the final whole-branch review's C1) — NOTHING that came from outside is written outside a fence, and a url is the PARSED href on a host the source actually serves.** R8's stated property is "prompt assembly never places external text outside the fence" and "every field that becomes a label is validated to a shape", and the branch broke it in two places at once. **(a) The url.** `safeUrl` (`packages/domain/src/external/github.ts`) returned the CALLER'S string after `new URL(value)` accepted it — and the WHATWG parser is specified to strip ASCII tab, LF and CR from its input *before* parsing, so `https://github.com/acme/x/issues/1\n\nSYSTEM: …  <<external-text>>` validated, was stored on `InboundEvent.payload.url` and `GoalVersion.origin.url`, and was written after the fence as `Source: <it>` — up to 499 characters of attacker-chosen prose on its own lines, un-neutralised (a url never went through `sanitiseExternalText`), able to spell a fence token or a `<slave-ask>` marker. `externalOriginSchema` held the field to a LENGTH and nothing else, so a stored one round-tripped. **(b) The subject's quote.** `sanitiseExternalText` spares `\n` and `\t` by design, because a quoted BODY reads as prose — and `composeAt` put its 120-code-point result into a ONE-LINE subject, so a title arriving with line breaks put attacker prose above the ask and outside any fence, reading as the operator's own requirement. **The fix, in four parts.** (1) `safeExternalUrl(value, source)` (`external/origin.ts`) answers `parsed.href` and never the input: under `EXTERNAL_URL_MAX_CHARS` (inclusive now in BOTH readers, which closes deferred item 3's `>= 500` versus `max(500)` disagreement), `https:` only, no `user:password@`, the host in `EXTERNAL_SOURCE_HOSTS` (`{ github: ['github.com'] }` — one small allow-list per source), and the serialised href itself under the cap and matching `EXTERNAL_URL_RE` (`https://` then printable ASCII with no space, quote or angle bracket, which a WHATWG-serialised href satisfies by construction). A `deployment_status.target_url` on another host is now DROPPED, deliberately: that field is set by whoever calls the deployments API, not by GitHub. (2) `externalOriginSchema.url` carries the same regex and a refinement requiring the stored value to equal `safeExternalUrl`'s own output for that origin's source, so no stored origin can round-trip one. (3) `composeExternalRequest` writes a subject that is GENERATED entirely — the kind's label, a middle dot, the validated repository and its validated ref, and nothing else — and ONE fence (not two: one preamble to read, one pair of tokens to get right) whose lines are `Title: <the sanitised 120-point quote>`, `Source: <the url, re-validated here rather than trusted from the caller>`, a blank line, and the sanitised body last, because the body is the only part E17's budget ever cuts. `fenceExternalBlock(lines)` in `external/fence.ts` is the one-fence builder; `fenceExternalText` is now a call to it, so there is still exactly one place the preamble and the two tokens are written. `EXTERNAL_REQUEST_FRAME_MAX_CHARS` is rederived from the new frame and is still proven smaller than `EXTERNAL_REQUEST_MAX_CHARS`. (4) The tests: a url carrying `\n`/`\t`/`\r` (validates under WHATWG stripping — asserted to be normalised away, not stored), a url whose path carries a `<<external-text>>` token (percent-encoded by the serialiser and inside the fence), a title carrying line breaks and imperative prose, the host/scheme/userinfo table, the cap's exact boundary in both readers, and a **probe** over eight adversarial vectors fed as the title, the body and the url in turn for all five kinds: everything outside the one fence is reconstructed from generated pieces alone and compared byte for byte, which states the property as an equality rather than as a search for needles. §3 stage 9 gains the same two statements at the level a gate can make them: the subject line character for character, and the number of LINES before the opening token (six, the last of them empty) with none of them carrying the delivery's own words. Demonstrated RED against `packages/domain/dist` the way the reviewer demonstrated it, then green.
- **E25 (amends R6, R10, §2's `ExternalIgnoredReason` and §3's stage list, from the final whole-branch review's I1) — a hook's secret is scoped to the repositories that hook speaks for.** R6 resolves the workspace from the delivery's own `repository.full_name` rather than from the hook, deliberately, so that an organisation-level hook delivering for several repositories reaches the right project each time. The consequence was recorded nowhere: `ingestExternalEvent` never compared the resolved `ExternalRepository` against the DELIVERING `hookId`, so a party holding the secret for `acme/a`'s hook could sign a delivery whose `repository.full_name` is `acme/b` and amend project B's requirement — even though the operator had given the two repositories different variables, which is the whole of what giving them different variables was supposed to mean. The resolved mapping's `hookId` must now equal `identity.hookId`; otherwise the delivery is recorded `ignored` with a FIFTH ignore reason, `hook_mismatch` — a row (so a forgery is visible, and it records which hook delivered it), attributed to NO project (the hook may not speak for that one, and `ExecutionEvent.workspaceId` is NOT NULL, so no event can be written either), no goal version, nothing moved. The organisation case is untouched and is tested: one `triggers map` per repository all naming ONE variable gives N hooks, each delivery arrives at the hook its own mapping issued, and the comparison is an identity. `hook_mismatch` is the one ignored reason that also earns R10's operator line — `[hooks] github delivery ignored: hook_mismatch`, written by the route through the new `hookIgnoredLine` (same bounding, same helper, and it says `ignored` rather than `refused` because the delivery WAS recorded) — because it is a fact about authorisation rather than about configuration; the other four stay the row's business alone, which is also fix-wave item 11's correction to a comment that promised a log line nobody wrote. The reason travels in the 200 response like the other four: a party who already holds a valid signing secret for this installation learns that the repository it named is mapped to some other hook, which is one bit more than `unmapped_repository` told them and is stated here rather than hidden by a response that disagrees with the row. **Costs a migration** — `20260913180000_m54_hook_mismatch`, one `ALTER TYPE "ExternalIgnoredReason" ADD VALUE IF NOT EXISTS 'hook_mismatch'`, additive, no data statement, the idiom every earlier enum widening in that directory uses — and the member is appended (not inserted) because that is what `ADD VALUE` does and `enum-parity` pins the two member for member. §3's stage list gains **stage 5b**, between 5 and 6 and renumbering nothing: a delivery signed with the mapped repository's secret, arriving at its hook, naming the SECOND mapping's repository, asserted to answer `200 {"status":"ignored","reason":"hook_mismatch"}` with one row carrying the delivering hookId and no workspace, zero new `ExecutionEvent` rows, `Workspace.goalVersion` unmoved, and the second mapping untouched.
