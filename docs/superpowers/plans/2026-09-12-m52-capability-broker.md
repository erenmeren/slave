...
- **E18 (amends R5/R7) — `grantedBy` and `permission.changed.by` hold a `User.id`, and every visible surface resolves it to a username AT ITS OWN BOUNDARY.** `setSlavePermission` writes `principal?.userId ?? null` onto `SlavePermission.grantedBy` and into the event's `by`, because `packages/control`'s `Principal` is `{ userId }` — the name is not in scope at the write, and adding it there would put a display string in a column every rule is joined on. Task 5's first pass printed those ids: the worker panel read `Granted by 9f3c1b2e-… on 12 Sep 2026` and the activity card read ` · by 9f3c1b2e-…`, while every fixture stated `meren`, so the suite could not see it. The rule is therefore stated here rather than solved in the writer: a surface that shows a granter resolves the id ONCE per page, at the boundary that knows names — `buildOverviewSnapshot` does one batched `user.findMany` over the roster's distinct `grantedBy` values and hands the panel `byName` beside `by`; the activity card reads the `userNameById` map `ActivityClient` already builds from `buildActivityPage`'s `users` listing, which is the same id and the same lookup `event.userId` already used. No per-worker and no per-card query. Two nulls, said differently: no principal at all (the CLI and the daemon carry none) prints `somebody unrecorded`, and a granter whose `User` row was deleted since prints `a person no longer on record`; the raw id is never visible text and always in `title` (`docs/ia.md` rule 3). **Still outstanding:** the CLI's `permission list` prints the id in its `by <username>` column — `apps/orchestrator` has a database and can resolve it, and it is fixed in the final wave rather than in Task 5's fix round, which is web-only.
